...
const TradeSection: React.FC<TradeSectionProps> = ({ rates, wallets, userWalletAddress, onOrderSubmit, isLoggedIn, onRequestLogin }) => {
  ...
  // Input Handling
  const [activeInput, setActiveInput] = useState<'dls' | 'crypto' | null>(null);
             ...
             <div className="max-w-2xl mx-auto flex flex-col pt-0">
                {step === 1 ? (
                  <div className="space-y-2 animate-in slide-in-from-right-4 fade-in duration-500">
                         ...
                         <div className="relative group">
                           ...
                           {selectedCrypto && (
                             <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col items-end pointer-events-none">
                                <span className="text-[10px] font-bold text-slate-400 mb-0.5">{shortName}</span>
                                <span className="text-sm font-bold text-white tracking-wide">≈ ${totalUSD.toFixed(2)}</span>
                             </div>
                           )}
                         </div>
                      </div>
                    ...
                    {/* Crypto Selector */}
                    <div>
                      {/* Live Market Rate Display - RESTORED HERE */}
                      {selectedCrypto && (
