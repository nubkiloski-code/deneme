...
const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const handleGoogleLogin = async () => {
    setIsSendingEmail(true); // Re-use loading state
    try {
      ...
      if (err.message === 'CONFIG_MISSING') {
        setError("Admin must configure Firebase keys in Settings.");
        setNotification({
          title: 'Configuration Error',
          message: 'Admin has not set up Firebase keys yet.'
        });
      } else {
        setError("Google Sign-In failed. See console.");
      }
    } finally {
              ...
              <div className="space-y-4">
                ...
                <div className="relative my-6">
                  ...
                  <div className="relative flex justify-center text-sm">
                    <span className="bg-gt-card px-2 text-slate-500">Or continue with</span>
                  </div>
                </div>
                ...
                <button 
                  ...
                  disabled={isSendingEmail}
                  className="w-full bg-white text-slate-900 font-bold py-3 rounded-xl hover:bg-slate-100 transition-colors flex items-center justify-center gap-3 disabled:opacity-50"
                >
